...
static ISD::Focuser *resolveFocuser(const QString &name, QString &error)
{
    if (!name.isEmpty())
    {
        ...
        if (!f) { error = QStringLiteral("Device '%1' is not a focuser").arg(name); return nullptr; }
        return f;
    }
...
void initFocuserTools(ToolRegistry *registry)
{
    ...
    registry->registerTool({
        ...
        [](const QJsonObject &args, QString &error) -> QJsonValue
        {
            auto focuser = resolveFocuser(args[QStringLiteral("name")].toString(), error);
            if (!focuser) return {};
            if (!focuser->canAbsMove()) { error = "Focuser does not support absolute moves"; return {}; }

            const int position = args[QStringLiteral("position")].toInt();
            if (position < 0) { error = "position must be >= 0"; return {}; }
            const uint32_t maxP = focuser->getmaxPosition();
            if (maxP > 0 && static_cast<uint32_t>(position) > maxP)
            { error = QStringLiteral("position %1 exceeds max %2").arg(position).arg(maxP); return {}; }

            if (!focuser->moveAbs(position)) { error = "Move request rejected by driver"; return {}; }
    ...
    registry->registerTool({
        ...
        [](const QJsonObject &args, QString &error) -> QJsonValue
        {
            auto focuser = resolveFocuser(args[QStringLiteral("name")].toString(), error);
            if (!focuser) return {};
            if (!focuser->canRelMove()) { error = "Focuser does not support relative moves"; return {}; }
            const int steps = args[QStringLiteral("steps")].toInt();
            if (steps == 0) { error = "steps must not be zero"; return {}; }
            ...
            if (!focuser->moveRel(std::abs(steps))) { error = "Move request rejected by driver"; return {}; }
            return QJsonObject { { QStringLiteral("success"), true } };
        }
    });

...
class Focus : public QWidget, public Ui::Focus
{
    ...
    public:
        ...
        // the MCP focus tools (task 28) to expose the HFR sample series for
        // in-session diagnosis. The vectors run in lock-step:
        ...
        const QVector<double> &getFocusPositions() const { return plot_position; }
        const QVector<double> &getFocusHFRValues() const { return plot_value;    }
        const QVector<double> &getFocusWeights()   const { return plot_weight;   }
        const QVector<bool>   &getFocusOutliers()  const { return plot_outlier;  }
